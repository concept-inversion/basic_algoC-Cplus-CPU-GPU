...
__shared__ int a[5];
        if(threadIdx.x==0){
         ...
         a[i]=i;}}
         int len=5;
         while(len/2>0 && threadIdx.x<len)
         {
         int required = len/2 + len%2;
         int offset = blockDim.x; 
         //printf("\nStart:%d, step:%d,stop:%d\n",start,step,stop);
         int halfpoint=(len/2);
         printf("Halfpoint:%d\n ",halfpoint);
                 for (int i =threadIdx.x; i<required;i+=offset)
                 {
                 int temp1=a[i];
                 int temp2=a[i+halfpoint];
                 __syncthreads();
                 ...
                 __syncthreads();
                 printf("thread: %d working on:%d and %d,value: %d\n",threadIdx.x,i,i+halfpoint,a[i]);
                }
                 __syncthreads();
                 len=halfpoint+len%2;
         }
         printf("Result: %d  thread%d \n", a[0],threadIdx.x);
